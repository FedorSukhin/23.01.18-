...
int main()
{
	...
	//tm* timeinfo;
	//time_t rtime;//переменная целочисленного типа
	//time(&rtime);//получаем текущую дату в виде целого числа
	//timeinfo=localtime(&rtime);//получаем текущую дату в виде объекта структуры
	...
	//timeinfo->tm_mday = 20;
	//mktime(timeinfo);//пересчитывает остальные поля структуры на основании внесённых изменений
	//cout << asctime(timeinfo) << "\n";
...
//1.Узнайте сколько дней осталось до Нового Года.
	...
	cout << newYear << "\n";
	cout<<newYear - currentDay<<" Дней до нового года!"<<"\n";*/

	//2.Узнайте сколько дней осталось до дня рождения пользователя. Если ДР уже был в этом году - 
	//вывести сообщение и написать, что ДР будет в следующем году.
	/*int day, month, currentDay, birthDay, res;
	cout << "Введите свой день рождения: \n";
	cout << "День: ";
	cin >> day;
	cout << "Месяц: ";
	cin >> month;
	...
	if (res > 0)
	{
		cout << "До дня рождения осталось " << res << " дней.\n";
	}
	else if (res == 0)
	{
		cout << "С ДР короче.\n";
	...
	{
		cout << "День рождения в следующем году.\n";
	}*/
	//3. Пользователь указывает год. Найдите все пятницы 13-е в этом году. Результат выведите в виде массива дат.
	/*int year, month, day;
